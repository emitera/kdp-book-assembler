...
import { Settings, BookOpen, Layers, Type, ChevronDown, ChevronUp, Sliders } from 'lucide-react';
...
export default function ConfigPanel() {
  ...
  const [activeSection, setActiveSection] = useState('general'); // 'general' | 'cover' | 'interior'
  ...
  const filteredTrimSizes = bindingType === 'hardcover'
    ? TRIM_SIZES.filter(size => allowedHardcoverSizes.includes(size.id))
    : TRIM_SIZES;
    ...
    <div className="space-y-4">
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl overflow-hidden shadow-xs">
        <button
          ...
          className="w-full flex items-center justify-between p-4 text-left font-bold text-slate-800 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-850/50 transition-all cursor-pointer"
        >
          <div className="flex items-center gap-2.5 text-xs uppercase tracking-wider">
            <BookOpen className="w-4 h-4 text-indigo-500" />
            <span>1. {t('config.section_general', 'Общие настройки (General)')}</span>
          </div>
          {activeSection === 'general' ? <ChevronUp className="w-4 h-4 text-slate-400" /> : <ChevronDown className="w-4 h-4 text-slate-400" />}
        </button>

        {activeSection === 'general' && (
          <div className="p-4 pt-0 border-t border-slate-100 dark:border-slate-800/80 space-y-4 animate-fade-in text-xs">
            ...
            <div className="flex flex-col space-y-1.5 pt-3">
              ...
              <div className="grid grid-cols-2 gap-2 bg-slate-50 dark:bg-slate-950 p-1 rounded-xl border border-slate-200 dark:border-slate-800">
                <button
                  ...
                  className={`py-2 px-3 rounded-lg text-[10px] font-bold transition-all cursor-pointer text-center ${
            ...
            <div className="flex flex-col space-y-1.5">
              ...
              >
                {filteredTrimSizes.map((size) => (
                  <option key={size.id} value={size.id}>
                    {size.name}
                  </option>
                ))}
                {bindingType === 'paperback' && (
                  <option value="custom">Custom size (Свой размер)...</option>
                )}
              </select>
            </div>

            {/* Custom Paperback Size Inputs */}
            {trimSizeId === 'custom' && bindingType === 'paperback' && (
              <div className="grid grid-cols-2 gap-3 p-3 bg-slate-50 dark:bg-slate-955 rounded-xl border border-slate-200 dark:border-slate-800">
                <div className="flex flex-col space-y-1">
                  <label className="text-[9px] text-slate-500 font-semibold uppercase">Ширина (in)</label>
                  <input
                    type="number"
                    step="0.01"
                    min="4"
                    max="12"
                    value={customWidth}
                    onChange={(e) => setCustomWidth(parseFloat(e.target.value) || 6.0)}
                    className="w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg px-2.5 py-1.5 text-xs font-semibold text-slate-800 dark:text-slate-200 focus:outline-none focus:border-indigo-500"
                  />
                </div>
                <div className="flex flex-col space-y-1">
                  <label className="text-[9px] text-slate-500 font-semibold uppercase">Высота (in)</label>
                  <input
                    type="number"
                    step="0.01"
                    min="4"
                    max="12"
                    value={customHeight}
                    onChange={(e) => setCustomHeight(parseFloat(e.target.value) || 9.0)}
                    className="w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg px-2.5 py-1.5 text-xs font-semibold text-slate-800 dark:text-slate-200 focus:outline-none focus:border-indigo-500"
                  />
                </div>
              </div>
      ...
      {/* SECTION 2: COVER CONFIGURATIONS */}
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl overflow-hidden shadow-xs">
        <button
          type="button"
        ...
        >
          <div className="flex items-center gap-2.5 text-xs uppercase tracking-wider">
            ...
            <span>2. {t('config.section_cover', 'Настройки обложки (Cover Settings)')}</span>
          </div>
          {activeSection === 'cover' ? <ChevronUp className="w-4 h-4 text-slate-400" /> : <ChevronDown className="w-4 h-4 text-slate-400" />}
        </button>
        ...
        {activeSection === 'cover' && (
          <div className="p-4 pt-0 border-t border-slate-100 dark:border-slate-800/80 space-y-4 animate-fade-in text-xs">
            ...
            <div className="flex flex-col space-y-1.5 pt-3">
              <label className="text-[10px] font-bold text-slate-455 dark:text-slate-400 uppercase tracking-wider">
                Режим обложки (Cover Layout)
              </label>
              <div className="grid grid-cols-2 gap-2 bg-slate-50 dark:bg-slate-950 p-1 rounded-xl border border-slate-200 dark:border-slate-800">
                <button
                  type="button"
                  onClick={() => setCoverType('parts')}
                  className={`py-2 px-3 rounded-lg text-[10px] font-bold transition-all cursor-pointer text-center ${
                    coverType === 'parts'
                      ? 'bg-white dark:bg-slate-800 text-indigo-650 dark:text-indigo-400 shadow-sm border border-slate-200/50 dark:border-slate-700/50'
                      : 'text-slate-500 hover:text-slate-700 dark:hover:text-slate-350'
            ...
            <div className="space-y-3 pt-2">
              <div className="flex items-center justify-between gap-4">
                <div className="space-y-0.5">
                  <label className="text-[10px] font-bold text-slate-455 dark:text-slate-400 uppercase tracking-wider block">
                    Цвет корешка
                  </label>
                </div>
                <div className="flex items-center gap-2">
                  ...
                  <input
                    ...
                    onChange={(e) => setSpineColor(e.target.value)}
                    className="w-8 h-8 border-0 p-0 rounded-lg overflow-hidden cursor-pointer bg-transparent"
                  />
                </div>
              </div>
              ...
              {textEligible ? (
                <div className="space-y-3 pt-3 border-t border-slate-100 dark:border-slate-800">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      <input
                        ...
                        className="w-20 px-2 py-1.5 border border-slate-205 dark:border-slate-855 bg-transparent rounded-lg text-xs text-slate-800 dark:text-slate-200 font-mono text-center focus:outline-none focus:border-indigo-500"
                      />
                      <input
                        type="color"
                  ...
                  <div className="flex flex-col gap-1.5 pt-1">
                    <span className="text-[10px] text-slate-450 uppercase tracking-wider font-semibold">Направление текста</span>
                ...
                <div className="p-3 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-855 rounded-xl text-center">
      ...
      {/* SECTION 3: INTERIOR CONFIGURATIONS */}
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl overflow-hidden shadow-xs">
        <button
          type="button"
        ...
        >
          <div className="flex items-center gap-2.5 text-xs uppercase tracking-wider">
            ...
            <span>3. {t('config.section_interior', 'Настройки страниц (Interior Settings)')}</span>
          </div>
          {activeSection === 'interior' ? <ChevronUp className="w-4 h-4 text-slate-400" /> : <ChevronDown className="w-4 h-4 text-slate-400" />}
        </button>
          ...
          <div className="p-4 pt-0 border-t border-slate-100 dark:border-slate-800/80 space-y-4 animate-fade-in text-xs">
            ...
            <div className="flex justify-between items-center py-2 pt-3">
              <div className="space-y-0.5">
                ...
                <span className="text-[9px] text-slate-400 block max-w-[200px]">
      ...
      {/* Book Metric Summary */}
      <div className="bg-gradient-to-br from-indigo-500 to-purple-650 rounded-2xl p-5 text-white space-y-4 shadow-md">
