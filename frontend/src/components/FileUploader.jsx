...
import { Upload, Trash2, CheckCircle2, AlertCircle, Image as ImageIcon } from 'lucide-react';
import { getImageDimensions, calculateRequiredDimensions, resizeImageToFit } from '../utils/imageValidator';
...
export default function FileUploader() {
  ...
  const [dragActiveCover, setDragActiveCover] = useState({ front: false, spine: false, back: false, interior: false });
  ...
  const checkAndProcessFile = async (file, uploadType, onSuccess) => {
    try {
      ...
      if (uploadType === 'front' || uploadType === 'back') {
        // Single page cover element: width + 0.125" bleed outer, height + 0.25" bleed top/bottom
        ...
        const spineWidth = interiorPages.length * activePaperType.multiplier;
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Grid of Compact Dropzones (4 Columns) */}
      <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
        
        {/* Dropzone 1: Front Cover */}
        <div className="flex flex-col">
          <span className="text-[10px] font-extrabold text-slate-500 uppercase tracking-wider mb-1">Передняя обложка</span>
          <div
              ...
              {frontCover ? (
                <>
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-[9px] text-slate-400">Лицо загружено</p>
                  </div>
                </>
              ) : (
                <>
        ...
        <div className="flex flex-col">
          ...
          {interiorPages.length >= 79 ? (
            ...
            <div className="flex items-center gap-2 px-3 h-14 border border-dashed border-slate-200 dark:border-slate-850 bg-slate-100/50 dark:bg-slate-950/30 rounded-xl opacity-60 cursor-not-allowed">
              ...
              <div className="min-w-0 flex-1">
                <span className="text-[10px] font-bold text-slate-400 block leading-tight">Корешок заблокирован</span>
                <span className="text-[9px] text-slate-400 block leading-none">Требуется от 79 страниц</span>
              </div>
            </div>
          )}
        </div>
        ...
        <div className="flex flex-col">
          <span className="text-[10px] font-extrabold text-slate-500 uppercase tracking-wider mb-1">Задняя обложка</span>
          <div
              ...
              {backCover ? (
                <>
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-[9px] text-slate-400">Оборот загружен</p>
                  </div>
                </>
              ) : (
                <>
      ...
      {interiorPages.length > 0 && (() => {
        const totalPages = (interiorPages.length * (isSingleSided ? 2 : 1)) + (addBlankAtStart ? 1 : 0);
        const rem = (totalPages + 1) % 4;
          ...
          <div className="p-4 bg-amber-50/60 dark:bg-amber-950/20 border border-amber-200/60 dark:border-amber-900/40 rounded-xl flex gap-3 text-xs text-amber-800 dark:text-amber-300 animate-fade-in shadow-2xs">
            ...
            <div className="space-y-1.5 leading-relaxed">
              ...
              <div className="bg-white/50 dark:bg-black/30 p-2.5 rounded-lg border border-amber-200/40 dark:border-amber-900/40 text-[11px] space-y-1">
                <div>Сейчас в вашей книге: <strong className="font-bold">{totalPages}</strong> стр.</div>
